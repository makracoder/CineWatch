...
async function fetchMovies(filters) {
    ...
    if (filters.genres.length > 0) {
        // User selected specific genres - use those
        selectedGenres = filters.genres;
    } else {
        // No genres selected - use mood-based suggestions
...
function getMoodGenres(mood, watchingWith) {
    ...
    let genres = moodGenreMap[mood] || [];
    
    if (watchingWithGenreMap[watchingWith]) {
        genres = [...new Set([...genres, ...watchingWithGenreMap[watchingWith]])];
    }
